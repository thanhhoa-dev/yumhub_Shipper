...
const HistoryTransaction = () => {
  ...
  useEffect(() => {
      const fetchData = async () => {
          try {
              ...
              const filteredTransactions = getListHistoryTransaction.walletShipper.filter(transaction => {
                  const transactionDate = new Date(transaction.time);
                  return transactionDate.getMonth() + 1 === monthFilter && transactionDate.getFullYear() === yearFilter;
              });
      ...
      return (
          <View style={styles.viewContentItemMain}>
              ...
              <View style={styles.viewTopContentItem}>
                  ...
                  <Text style={styles.textTimeHistoryItem}>{formatDate(item.time)}</Text>
                  <View style={styles.viewFilter}>
                      {(item.nameBank !== undefined) ? (
                          <View style={styles.viewFilter}><Image
                              style={styles.imageLogoBank}
                              source={{
  ...
  return (
      <View style={styles.container}>
          ...
          <View style={styles.viewItemMain}>
              ...
              >
                  ...
                  <Text style={styles.txtOver}>
                      {filterList(monthFilter, yearFilter, listTopUp.reverse()).length > 0 ? "bạn đã xem hết các lịch sử giao dịch" : "bạn không có giao dịch nào trong thời gian này"}
                  </Text>
          ...
          >
              <PickerMonthYear
                  monthFilter={monthFilter} yearFilter={yearFilter} setisShowPickerTime={setisShowPickerTime}
                  setMonthFilter={setMonthFilter} setYearFilter={setYearFilter}
              />
          </Modal>
      </View>
  );
